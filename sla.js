const slaView = (a => {
	...
	(a => {
		...
		el({a:'option', b:a, c:'SLA Performance', d:{value:'sla_performance', selected:true} })
...
const sla = a => {
	...
	document.body.appendChild(slaView)
	...
	// update data
	m.sla.chart1.data.labels = ['Jul', 'Jun', 'Mei', 'Apr', 'Mar', 'Feb', 'Jan']
	m.sla.chart1.data.datasets.forEach((_, a) => {
		m.sla.chart1.data.labels.forEach(_ => { m.sla.chart1.data.datasets[a].data.push(Math.round(Math.random() * 100)) })
	})
	m.sla.chart1.update()
